use std::{convert::TryInto, ptr, mem};
use std::f64;
use rayon::{prelude::*};
use half::f16;
...
#[no_mangle]
pub extern "C" fn fill_plane(vert_buf: ptr::NonNull<Vertex>, indx_buf: ptr::NonNull<Quad>, side_len: usize) -> *const u8 {
    ...
    fill_plane_buffers(v_buffer, indx_buffer, side_len);

    "OK\0".as_ptr()
}

fn fill_plane_buffers(v_buffer: &mut [Vertex], indx_buffer: &mut [Quad], side_len: usize) {
...
pub extern "C" fn get_plane(vptr: *mut *const Vertex, iptr: *mut *const Quad, side_len: usize) -> usize {
    ...
    fill_plane_buffers(v_buffer.as_mut_slice(), indx_buffer.as_mut_slice(), side_len);
